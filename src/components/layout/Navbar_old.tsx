...
function MegaMenuHebergement({ categories, isScrolled }: { categories: CategorieHebergement[]; isScrolled: boolean }) {
  ...
  const color = isScrolled ? "#1A1A1A" : "#FFFFFF";
...
export default function Navbar({ categories }: NavbarProps) {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 w-full z-[60] transition-all duration-500 ${scrolled ? "bg-dar-sand/95 backdrop-blur-md shadow-sm" : "bg-transparent"}`}
        ...
        <div className="flex-none ml-auto flex items-center gap-4">
          ...
          <button onClick={() => setIsOpen(!isOpen)} className="relative z-[70] p-2 md:hidden" aria-label="Menu">
            <div className="flex flex-col justify-between w-7 h-5">
              <motion.span animate={isOpen ? { rotate: 45, y: 9 } : { rotate: 0, y: 0 }} className={`block h-0.5 w-full transition-colors duration-300 ${scrolled || isOpen ? "bg-dar-dark" : "bg-white"}`} />
              <motion.span animate={isOpen ? { opacity: 0 } : { opacity: 1 }} className={`block h-0.5 w-full transition-colors duration-300 ${scrolled || isOpen ? "bg-dar-dark" : "bg-white"}`} />
              <motion.span animate={isOpen ? { rotate: -45, y: -9 } : { rotate: 0, y: 0 }} className={`block h-0.5 w-full transition-colors duration-300 ${scrolled || isOpen ? "bg-dar-dark" : "bg-white"}`} />
            </div>
          </button>
        </div>
