...
import type { Chambre } from "../../../lib/chambres";

interface ChambresListProps {
  chambres: Chambre[];
}
...
function ChambreCard({ chambre, index }: ChambreCardProps) {
      ...
      >
        ...
        >
          <div className="relative w-full" style={{ minHeight: "320px", height: "100%" }}>
            <Image
              src={chambre.images[0].src}
              alt={chambre.images[0].alt}
              fill
            ...
            >
              ...
              >
                À partir de {chambre.priceFrom.toLocaleString("fr-FR")} €
              </span>
              <span
                className="ml-1 text-xs"
                style={{ fontFamily: "var(--font-inter), sans-serif", color: "#A3895D" }}
              >
                / nuit
              </span>
            </div>
        ...
        >
          ...
          >
            {[
              { label: "Surface", value: `${chambre.surface} m²` },
              { label: "Altitude", value: chambre.altitude },
              { label: "Vue", value: chambre.view },
              { label: "Capacité", value: `${chambre.capacity} pers.` },
            ].map((spec) => (
              <div key={spec.label}>
                <p
          ...
          <div className="flex items-center gap-4">
            <Link
              ...
              style={{ backgroundColor: "#1A1A1A", borderRadius: "2px", textDecoration: "none", flexShrink: 0 }}
            >
              <span
            ...
            <Link
              ...
              style={{ color: "rgba(26,26,26,0.5)", fontFamily: "var(--font-inter), sans-serif", textDecoration: "none" }}
            >
              Réserver
            </Link>
          </div>
...
function ChambresIntro() {
  ...
  return (
    ...
    >
      ...
      >
        Des chambres où l'artisanat berbère dialogue avec le confort contemporain
      </motion.h2>
      ...
      <motion.div
        ...
        className="mt-8"
        style={{ width: "40px", height: "1px", backgroundColor: "#A3895D", transformOrigin: "left" }}
      />
    </div>
  );
}

export default function ChambresList({ chambres }: ChambresListProps) {
  return (
    <section style={{ backgroundColor: "#FDFBF7" }}>
      <ChambresIntro />

      <div style={{ width: "100%", height: "1px", backgroundColor: "rgba(26,26,26,0.08)" }} />

      {chambres.map((chambre, i) => (
        <ChambreCard key={chambre.slug} chambre={chambre} index={i} />
      ))}
      ...
      >
        ...
        <Link
          ...
          style={{ border: "1px solid rgba(253,251,247,0.25)", borderRadius: "2px", textDecoration: "none" }}
        >
          <span
            className="absolute inset-0 origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500 ease-out"
            style={{ backgroundColor: "#EA580C" }}
