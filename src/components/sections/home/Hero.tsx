    ...
    <section className="relative h-screen w-full overflow-hidden">
      ...
      >
        ...
        <motion.div variants={fadeUp} className="mt-10 flex items-center justify-center gap-6">
          <Link
            ...
            style={{ backgroundColor: "#EA580C", borderRadius: "2px" }}
          >
            <span
      ...
      >
        {[
          ...
          <motion.div key={stat.label} variants={fadeIn} className="flex flex-col items-end">
            <span style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "1.8rem", fontWeight: 300, color: "#FDFBF7", lineHeight: 1 }}>
              {stat.value}
            </span>
            <span className="text-xs tracking-[0.15em] uppercase mt-1" style={{ color: "rgba(253,251,247,0.45)", fontFamily: "var(--font-inter), sans-serif" }}>
              {stat.label}
            </span>
          </motion.div>
        ))}
      ...
      >
        <motion.div
          style={{ width: "1px", height: "60px", background: "linear-gradient(to bottom, rgba(253,251,247,0.5), transparent)" }}
          animate={{ scaleY: [1, 0.5, 1], originY: 0 }}
          transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" as const }}
        />
        <span className="text-xs tracking-[0.25em] uppercase" style={{ color: "rgba(253,251,247,0.4)", fontFamily: "var(--font-inter), sans-serif", writingMode: "vertical-rl", letterSpacing: "0.2em" }}>
          Défiler
        </span>
      </motion.div>
    </section>
